            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 p-4 bg-red-50 rounded-lg">
                        <h3 class="text-lg font-medium text-red-900 mb-2">{{ \App\Helpers\WorkflowHelper::getRoleFullForm('dr') }} Remarks</h3>
                        <div class="text-sm text-gray-700 whitespace-pre-wrap mb-2">{{ $report->dr_remarks }}</div>
                        @if($report->drApprover)
                        <div class="text-xs text-gray-600 mt-1">
                            <strong>Approved by:</strong> {{ $report->drApprover->name }} on {{ $report->dr_approved_at->format('M d, Y H:i') }}
                        </div>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="mt-8 p-6 bg-white border border-gray-200 rounded-lg">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">{{ \App\Helpers\WorkflowHelper::getRoleFullForm('hvc') }} Remark</h3>
                        <form method="POST" action="{{ route('hvc.progress_reports.process', $report) }}">
                            @csrf
                            @method('POST')
                            ...
                            <div id="reassignment_fields" class="mb-6 hidden">
                                <div class="mb-4">
                                    ...
                                    <select id="reassigned_to_role" name="reassigned_to_role" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        <option value="dr" {{ old('reassigned_to_role') == 'dr' ? 'selected' : '' }}>{{ \App\Helpers\WorkflowHelper::getRoleFullForm('dr') }}</option>
                                    </select>
                            ...
                            <div class="flex items-center justify-end space-x-4">
                                <a href="{{ route('hvc.progress_reports.pending') }}"
