    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-yellow-500 rounded-lg p-6 text-white">
            <div class="flex items-center">
                ...
                <div>
                    <div class="text-2xl font-bold">{{ $statistics['pending_registrations'] }}</div>
                    <div class="text-yellow-100">Pending Registrations</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-green-500 rounded-lg p-6 text-white">
            <div class="flex items-center">
                ...
                <div>
                    <div class="text-2xl font-bold">{{ $statistics['signed_registrations'] }}</div>
                    <div class="text-green-100">Signed Registrations</div>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('dr.synopsis.approve', $synopsis) }}"
    ...
    @if($pending_registrations->count() > 0)
        <div class="bg-white dark:bg-gray-700 rounded-lg shadow p-6">
            <h3 class="text-xl font-semibold mb-4 flex items-center">
                ...
                Pending Registration Forms (Generated by DA)
            </h3>
                ...
                @foreach($pending_registrations as $registration)
                    <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <h4 class="font-medium text-gray-900 dark:text-white">
                                    {{ $registration->scholar->user->first_name }} {{ $registration->scholar->user->last_name }}
                                </h4>
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    Dispatch Number: {{ $registration->dispatch_number }}
                                </p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    Generated on {{ $registration->generated_at->format('M d, Y') }}
                                </p>
                            </div>
                            <div class="flex space-x-2">
                                <a href="{{ route('registration.sign.dr', $registration) }}"
