                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($scholars as $scholar)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $scholar->admission->department->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ ucfirst(str_replace('_', ' ', $scholar->status)) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('dean.scholars.show', $scholar) }}" class="text-indigo-600 hover:text-indigo-900">View Details</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
