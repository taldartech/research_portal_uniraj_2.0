<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('All Coursework Exemption Requests') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="mb-6">
                        <h3 class="text-lg font-medium text-gray-900">All Coursework Exemption Requests</h3>
                        <p class="text-sm text-gray-600">Complete list of all coursework exemption requests with their current status</p>
                    </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($exemptions as $exemption)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ $exemption->scholar->admission->department->name ?? 'N/A' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @php
                                                    ...
                                                    $colorClass = $statusColors[$exemption->status] ?? 'bg-gray-100 text-gray-800';
                                                @endphp
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                @if($exemption->supporting_documents)
                                                    <a href="{{ Storage::url($exemption->supporting_documents) }}"
                                                       target="_blank"
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No coursework exemption requests</h3>
                            <p class="mt-1 text-sm text-gray-500">There are no coursework exemption requests at this time.</p>
                        </div>
                    @endif
                </div>
            </div>
