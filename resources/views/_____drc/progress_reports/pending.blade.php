<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Pending Progress Reports for DRC Approval') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    @if ($pendingReports->isEmpty())
                        <p>No pending progress reports for DRC approval.</p>
                    @else
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($pendingReports as $report)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $report->supervisor->user->name }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('drc.progress_reports.approve', $report) }}"
                                               class="text-indigo-600 hover:text-indigo-900 mr-3">
                                                Review & Approve
                                            </a>
                                            <a href="{{ route('drc.progress_reports.show', $report) }}"
