<x-guest-layout>
    ...
    @if(!isset($otp_sent) || !$otp_sent)
        ...
        <form method="POST" action="{{ route('scholar.send-otp') }}">
            @csrf
    ...
    @else
        ...
        <form method="POST" action="{{ route('scholar.login') }}">
            ...
            <div class="flex items-center justify-end mt-4">
                <a href="{{ route('scholar.login') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 mr-3">
                    {{ __('Back') }}
                </a>
                <x-primary-button>
                    {{ __('Log in as Scholar') }}
                </x-primary-button>
            </div>
        </form>
    @endif
